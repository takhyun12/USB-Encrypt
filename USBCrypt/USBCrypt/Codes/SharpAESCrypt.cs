...
namespace SharpAESCrypt
{
    ...
    public class SharpAESCrypt : Stream
    {
        ...
        /// </summary>
        public static string Extension_CreatedByIdentifier = string.Format("ROKAF v{0}", System.Reflection.Assembly.GetExecutingAssembly().GetName().Version);
        ...
        public SharpAESCrypt(string password, Stream stream, OperationMode mode)
        {
            ...
            if (mode == OperationMode.Encrypt)
            {
                ...
                if (Extension_InsertCreateByIdentifier)
                    m_extensions.Add(new KeyValuePair<string, byte[]>("ENCRYPT-BY", Encoding.UTF8.GetBytes(Extension_CreatedByIdentifier)));
        ...
        public static void AESCrypt(string[] args)
        {
            if (args.Length < 4)
            {
